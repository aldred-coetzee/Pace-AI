"""E2E tests for withings-mcp server startup.
...
class TestServerStartupE2E:
    def test_withings_mcp_boots_and_responds(self):
        """Start withings-mcp as a subprocess and verify it serves HTTP."""
        port = _find_free_port()
        env = {
            "WITHINGS_MCP_PORT": str(port),
            "PATH": "",
        }
        full_env = {**os.environ, **env}

        proc = subprocess.Popen(
            [sys.executable, "-m", "withings_mcp.server"],
            env=full_env,
        ...
        try:
            ...
            assert started, f"withings-mcp failed to start on port {port}"
